# CNN for 3D Underwater Acoustic Field Prediction

This repository provides a self-contained conditional CNN example for predicting 3-D underwater acoustic transmission-loss (TL) fields. The model takes a 52-dimensional environmental vector and a 3-D geo-steady background field as inputs and predicts a TL field with shape `[4, 36, 250]`.

## Data setup

The code constructs 18,876 logical samples at runtime:
...
The default CNN configuration is:
...
The 52-dimensional environmental vector is encoded by an MLP. The encoded condition is mapped to feature-wise scale and bias parameters, which modulate convolutional features extracted from the geo-steady TL background field. The modulated features are processed by eight residual blocks followed by deeper convolutional layers, and the network output shape is `[4, 36, 250]`.

The default model contains 11,065,508 parameters.
...
model.py           conditional CNN model
losses.py          MixedLoss
...
Training outputs are saved to `outputs_cnn_eofk/` by default, including:
