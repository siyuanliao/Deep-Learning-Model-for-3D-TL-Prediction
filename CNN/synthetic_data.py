...
but they are not a replacement for Bellhop3D simulations.
"""
...
def generate_tl(indices, cfg=DATA):
    ...
    # Avoid in-place broadcasting: anisotropy is [N,1,A,1] before range modulation,
    # while the range factor is [1,1,1,R]. A normal multiplication can expand
    # the result to [N,1,A,R]; in-place '*=' cannot change the left-hand shape.
    anisotropy = anisotropy * (1.0 - np.exp(-r / 45.0))
